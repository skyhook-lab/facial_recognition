...
class FacesTracker extends ChangeNotifier {
  ...
  Image? _fsdkImage;
  bool _savedForCurrentMatch = false;
  ...
  void _initialize() async {
    ...
    _receive.listen((msg) async {
      ...
      _fsdkImage = null;
      final hasMatch = _ids.any((id) => getNameForId(id).isNotEmpty);
      if (!hasMatch) {
        _savedForCurrentMatch = false;
      }

      if (imageToSave != null && hasMatch && !_savedForCurrentMatch) {
        try {
          Directory directory = await getApplicationDocumentsDirectory();
          String fileName = '${directory.path}/frame.jpg';
          print('Saving frame to $fileName');
          imageToSave.saveToFile(fileName);
          print('Frame saved to $fileName');
          _savedForCurrentMatch = true;
        } finally {
          imageToSave.free();
        }
      } else {
  ...
  FaceMatchResult matchFace(Image img) {
    ...
    var similarityResults = _tracker.matchFaces(faceTemplate, 0.992);
